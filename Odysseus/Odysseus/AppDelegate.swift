...
class AppDelegate: UIResponder, UIApplicationDelegate {
	...
	/**
	 Do Some stuff on the global UI
	 */
	func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
		// Override point for customization after application launch.
		let themecolor = UIColor(red: 255 / 255, green: 81 / 255, blue: 70 / 255, alpha: 1.0)
		...
		UIPageControl.appearance().tintColor = UIColor.lightGrayColor()
		UIPageControl.appearance().backgroundColor = UIColor.clearColor()
		UILabel.appearance().substituteFontName = "Futura"
		UITextView.appearance().substituteFontName = "Futura"
		UIButton.appearance().substituteFontName = "Futura"
		UITabBarItem.appearance().setTitleTextAttributes(
			[NSFontAttributeName: UIFont(name: "Futura", size: 11)!],
			forState: .Normal)
		let customFont = UIFont(name: "Futura", size: 17.0)
	...
	var persistentStoreCoordinator: NSPersistentStoreCoordinator! {
		if _persistentStoreCoordinator == nil {
			...
			do {
				try _persistentStoreCoordinator!.addPersistentStoreWithType(NSSQLiteStoreType, configuration: nil, URL: storeURL, options: [NSPersistentStoreUbiquitousContentNameKey: "MyAppCloudStore"])
			} catch {
				abort()
		} }
